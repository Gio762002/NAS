...
def as_auto_addressing_for_link(dict_as):
    ...
    for As in dict_as.values(): 
        ...
        for ((r1, i1),(r2, i2)) in As.link_dict.items():
            ...
            router1 = get_router_instance(r1, dict_as)
            router2 = get_router_instance(r2, dict_as)
            if router1.interfaces.get(i1).address_ipv4 is None:
                router1.interfaces.get(i1).address_ipv4 = addresses[0]
            if router2.interfaces.get(i2).address_ipv4 is None:
                router2.interfaces.get(i2).address_ipv4 = addresses[1]
...
def reset_interface(router,int):
    ...
    router.interfaces[int].address_ipv4 = None
